...
class Base:
    def __init__(self, host='', page=1, endPage=1):
        self._host = host
        self._page = page
        self._endPage = endPage + 1
        self._travelData = []  # 旅遊資料
        self._flightData = []  # 航班資料
    ...
    def crawler(self, page):
        ...
        url = '{}/EW/Services/SearchListData.asp'.format(self._host)
        headers = {
    ...
    def filghtCrawler(self, siteTitle, prodCd, sacctNo):
        url = '{}/EW/Services/SearchFlight.asp?prodCd={}&sacctNo={}&flightType=1'
        url = url.format(self._host, prodCd, sacctNo)
...
class OrangeTravel(Base):
    def __init__(self, page=1, endPage=1):
        host = 'http://www.orangetour.com.tw'
        super().__init__(host, page, endPage)


class NewamazingTravel(Base):
    def __init__(self, page=1, endPage=1):
        host = 'https://www.newamazing.com.tw'
        super().__init__(host, page, endPage)


class SaveCsv:
    def __init__(self, csvPath):
        self._csvFile = open(csvPath, 'w', newline='')
        self._csv_file_writer = csv.writer(self._csvFile)

    def writeRow(self, row):
        self._csv_file_writer.writerow(row)

    def writeRows(self, rowList):
        self._csv_file_writer.writerows(rowList)
...
class MainApp:
    def __init__(self, page, endPage):
        ...
        saveCsv = SaveCsv(travelCsvPath)
        saveCsv.writeRow(travelHead)
        saveCsv.writeRows(orangeTravel.travelData)
        saveCsv.writeRows(newamazingTravel.travelData)
        saveCsv.close()
        ...
        flightHead = ['旅行社',
                      ...
                      '目的地', '航空公司']
        saveCsv = SaveCsv(flightCsvPath)
        saveCsv.writeRow(flightHead)
        saveCsv.writeRows(orangeTravel.flightData)
        saveCsv.writeRows(newamazingTravel.flightData)
        saveCsv.close()
